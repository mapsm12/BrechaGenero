...
SOURCE_DOCS = {
    "ENAHO": {
        "name": "Encuesta Nacional de Hogares (ENAHO)",
        "institution": "Instituto Nacional de Estadística e Informática (INEI)",
        "description": (
            "Encuesta continua de hogares del INEI. En el dashboard se usan resultados anuales con "
            "desagregación departamental y factores de expansión (PESO) para producir estimaciones ponderadas."
        ),
        "coverage": "Perú, años 2008–2024, uso analítico anual y comparación territorial.",
        "note": (
            "El INEI documenta que los factores de expansión incorporan el diseño muestral y ajustes por no entrevista; "
            "además, la ENAHO permite inferencia anual a nivel departamental."
        ),
    },
    "JNE": {
        "name": "Autoridades electas",
        "institution": "Jurado Nacional de Elecciones (JNE) / Plataforma Nacional de Datos Abiertos",
        "description": (
            "Base de autoridades electas usada para construir indicadores de representación y liderazgo político femenino."
        ),
        "coverage": (
            "Se agregan registros por departamento y año electoral usando campos como CARGO, AMBITO, UBIGEO, "
            "ANIOELECCION, GENERO y ORGANIZACIONPOLITICA."
        ),
        "note": (
            "En este app las métricas JNE se derivan por conteos y proporciones sobre autoridades electas, "
            "según la definición operacional aplicada en tu preproceso."
        ),
    },
    "COMBINED": {
        "name": "Métricas integradas ENAHO + JNE",
        "institution": "Derivación analítica del proyecto",
        "description": (
            "Índices y comparaciones construidos combinando entorno socioeconómico, cuidados y representación política."
        ),
        "coverage": "Comparación departamental y temporal dentro del propio dashboard.",
        "note": (
            "No es una variable oficial publicada por una entidad; es una construcción analítica reproducible a partir de fuentes oficiales."
        ),
    },
}

VARIABLE_DOCS = {
    "PESO": "Factor de expansión / peso muestral de la ENAHO.",
    "SEXO": "Sexo registrado de la persona (1=hombre, 2=mujer en la base analítica).",
    "EDAD": "Edad de la persona en años.",
    "TRABAJA_HORAS": "Indicador operativo del dashboard: 1 si HORAS_TRABAJADAS_TOTAL > 0, 0 en otro caso.",
    "HORAS_TRABAJADAS_TOTAL": "Horas trabajadas reportadas por la persona.",
    "POBRE_BIN": "Indicador binario derivado de la condición de pobreza en la base analítica.",
    "INGRESO_NETO_HOGAR": "Ingreso neto del hogar asociado al registro individual en la base analítica.",
    "ES_EDUC_SUPERIOR": "Indicador derivado de educación superior alcanzada.",
    "ES_EDUC_UNIVERSITARIA": "Indicador derivado de educación universitaria alcanzada.",
    "TIENE_NINIO_U6_HOGAR": "Proxy de cuidados intensivos: 1 si el hogar tiene al menos un niño o niña menor de 6 años.",
    "N_NINIOS_U6_HOGAR": "Número de niños/as menores de 6 años en el hogar.",
    "GENERO": "Sexo/género registrado en el dataset de autoridades electas del JNE.",
    "CARGO": "Cargo de la autoridad electa (alcaldía, regiduría, gobernación, etc.).",
    "AMBITO": "Ámbito del cargo en el dataset JNE (regional, provincial, distrital).",
    "UBIGEO": "Código territorial usado para agregación geográfica.",
    "ANIOELECCION": "Año del proceso electoral considerado.",
    "ORGANIZACIONPOLITICA": "Organización política de la autoridad electa.",
}


def metric_source_key(metric_id):
    if metric_id in INDEX_METRIC_IDS:
        return "COMBINED"
...
def _md_bullets(items):
    return "\n".join([f"- **{k}**: {v}" for k, v in items]) if items else "- No aplica"


def infer_metric_description(metric_id):
    if metric_id.startswith("tasa_trabaja_horas_"):
        grupo = "mujeres" if metric_id.endswith("mujer") else "hombres"
        return f"Proporción ponderada de {grupo} de 18 a 64 años que reportan trabajo efectivo medido por horas (TRABAJA_HORAS = 1)."
    if metric_id.startswith("tasa_educ_sup_"):
        grupo = "mujeres" if metric_id.endswith("mujer") else "hombres"
        return f"Proporción ponderada de {grupo} de 18 a 64 años con educación superior según la variable derivada ES_EDUC_SUPERIOR."
    if metric_id.startswith("tasa_educ_univ_"):
        grupo = "mujeres" if metric_id.endswith("mujer") else "hombres"
        return f"Proporción ponderada de {grupo} de 18 a 64 años con educación universitaria según la variable derivada ES_EDUC_UNIVERSITARIA."
    if metric_id.startswith("horas_prom_"):
        grupo = "mujeres" if metric_id.endswith("mujer") else "hombres"
        return f"Promedio ponderado de horas trabajadas entre {grupo} ocupados/as de 18 a 64 años."
    if metric_id.startswith("pobreza_") and not metric_id.startswith("penalidad_"):
        grupo = "mujeres" if metric_id.endswith("mujer") else "hombres"
        return f"Proporción ponderada de {grupo} de 18 a 64 años en condición de pobreza según la codificación de la base analítica ENAHO."
    if metric_id.startswith("ingreso_neto_hogar_"):
        grupo = "mujeres" if metric_id.endswith("mujer") else "hombres"
        return f"Promedio ponderado del ingreso neto del hogar asociado a {grupo} de 18 a 64 años."
    if metric_id == "share_mujeres_18_49_con_u6":
        return "Proporción ponderada de mujeres de 18 a 49 años que viven en hogares con al menos un niño o niña menor de 6 años."
    if metric_id == "prom_ninios_u6_hogar_mujer_18_49":
        return "Promedio ponderado del número de niños/as menores de 6 años en hogares de mujeres de 18 a 49 años."
    if metric_id == "share_mujeres_18_49_con_2mas_u6":
        return "Proporción ponderada de mujeres de 18 a 49 años que viven en hogares con dos o más niños/as menores de 6 años."
    if metric_id.startswith("gap_"):
        return "Brecha calculada como diferencia aritmética entre dos grupos o condiciones; el signo indica la dirección de la ventaja o desventaja."
    if metric_id.startswith("ratio_"):
        return "Razón entre dos grupos o condiciones; el valor 1 indica equilibrio relativo."
    if metric_id.startswith("penalidad_maternidad_"):
        return "Diferencia entre mujeres de 18–49 años con U6 en el hogar y mujeres de 18–49 años sin U6 en el hogar."
    if metric_id.startswith("mitigacion_penalidad_"):
        return "Diferencia entre penalidades estimadas por nivel educativo; aproxima cuánto cambia la penalidad asociada a maternidad proxy cuando media la educación superior."
    if metric_id.startswith("share_") and metric_id in JNE_METRIC_IDS:
        return "Proporción de mujeres dentro del universo político definido por la métrica, agregada por departamento y año electoral a partir del dataset de autoridades electas."
    if metric_id == "ratio_autoridades_mujer_hombre":
        return "Razón entre número de autoridades mujeres y número de autoridades hombres en el universo político observado."
    if metric_id == "indice_techo_politico_mujeres":
        return "Índice de techo político: compara la representación femenina en cargos de liderazgo respecto a su representación femenina total entre autoridades electas."
    if metric_id == "brecha_liderazgo_vs_base_mujeres":
        return "Brecha entre la presencia femenina en liderazgo y la presencia femenina total dentro del conjunto de autoridades electas."
    if metric_id == "indice_habilitante_liderazgo_femenino_0_100":
        return "Índice compuesto del proyecto que resume condiciones socioeconómicas, educativas, de cuidados y representación política favorables al liderazgo femenino."
    return "Métrica derivada del procesamiento analítico del proyecto."


def infer_metric_formula(metric_id):
    if metric_id.startswith("tasa_trabaja_horas_"):
        sexo = "2" if metric_id.endswith("mujer") else "1"
        return (
            "p_w = Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64, TRABAJA_HORAS_i=1)] / "
            "Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64)]"
        )
    if metric_id.startswith("tasa_educ_sup_"):
        sexo = "2" if metric_id.endswith("mujer") else "1"
        return (
            "p_w = Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64, ES_EDUC_SUPERIOR_i=1)] / "
            "Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64)]"
        )
    if metric_id.startswith("tasa_educ_univ_"):
        sexo = "2" if metric_id.endswith("mujer") else "1"
        return (
            "p_w = Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64, ES_EDUC_UNIVERSITARIA_i=1)] / "
            "Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64)]"
        )
    if metric_id.startswith("horas_prom_"):
        sexo = "2" if metric_id.endswith("mujer") else "1"
        return (
            "x̄_w = Σ_i [w_i · HORAS_TRABAJADAS_TOTAL_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64, TRABAJA_HORAS_i=1)] / "
            "Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64, TRABAJA_HORAS_i=1)]"
        )
    if metric_id.startswith("pobreza_") and not metric_id.startswith("penalidad_"):
        sexo = "2" if metric_id.endswith("mujer") else "1"
        return (
            "p_w = Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64, POBRE_BIN_i=1)] / "
            "Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64)]"
        )
    if metric_id.startswith("ingreso_neto_hogar_"):
        sexo = "2" if metric_id.endswith("mujer") else "1"
        return (
            "x̄_w = Σ_i [w_i · INGRESO_NETO_HOGAR_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64)] / "
            "Σ_i [w_i · I(SEXO_i=" + sexo + ", 18≤EDAD_i≤64)]"
        )
    if metric_id == "gap_trabaja_horas_h_m":
        return "gap = tasa_trabaja_horas_hombre − tasa_trabaja_horas_mujer"
    if metric_id == "gap_horas_h_m":
        return "gap = horas_prom_hombre − horas_prom_mujer"
    if metric_id == "gap_pobreza_m_h":
        return "gap = pobreza_mujer − pobreza_hombre"
    if metric_id == "gap_ingreso_hogar_h_m":
        return "gap = ingreso_neto_hogar_hombre − ingreso_neto_hogar_mujer"
    if metric_id == "gap_educ_sup_m_h":
        return "gap = tasa_educ_sup_mujer − tasa_educ_sup_hombre"
    if metric_id == "gap_educ_univ_m_h":
        return "gap = tasa_educ_univ_mujer − tasa_educ_univ_hombre"
    if metric_id == "ratio_trabajo_m_h":
        return "ratio = tasa_trabaja_horas_mujer / tasa_trabaja_horas_hombre"
    if metric_id == "ratio_horas_m_h":
        return "ratio = horas_prom_mujer / horas_prom_hombre"
    if metric_id == "ratio_ingreso_hogar_m_h":
        return "ratio = ingreso_neto_hogar_mujer / ingreso_neto_hogar_hombre"
    if metric_id == "ratio_pobreza_m_h":
        return "ratio = pobreza_mujer / pobreza_hombre"
    if metric_id == "ratio_educ_sup_m_h":
        return "ratio = tasa_educ_sup_mujer / tasa_educ_sup_hombre"
    if metric_id == "share_mujeres_18_49_con_u6":
        return "p_w = Σ_i [w_i · I(SEXO_i=2, 18≤EDAD_i≤49, TIENE_NINIO_U6_HOGAR_i=1)] / Σ_i [w_i · I(SEXO_i=2, 18≤EDAD_i≤49)]"
    if metric_id == "prom_ninios_u6_hogar_mujer_18_49":
        return "x̄_w = Σ_i [w_i · N_NINIOS_U6_HOGAR_i · I(SEXO_i=2, 18≤EDAD_i≤49)] / Σ_i [w_i · I(SEXO_i=2, 18≤EDAD_i≤49)]"
    if metric_id == "share_mujeres_18_49_con_2mas_u6":
        return "p_w = Σ_i [w_i · I(SEXO_i=2, 18≤EDAD_i≤49, N_NINIOS_U6_HOGAR_i≥2)] / Σ_i [w_i · I(SEXO_i=2, 18≤EDAD_i≤49)]"
    if metric_id == "penalidad_maternidad_trabajo":
        return "penalidad = tasa_trabaja_mujer_si_u6 − tasa_trabaja_mujer_no_u6"
    if metric_id == "penalidad_maternidad_horas":
        return "penalidad = horas_mujer_si_u6 − horas_mujer_no_u6"
    if metric_id == "penalidad_maternidad_pobreza":
        return "penalidad = pobreza_mujer_si_u6 − pobreza_mujer_no_u6"
    if metric_id == "penalidad_maternidad_ingreso":
        return "penalidad = ingreso_hogar_mujer_si_u6 − ingreso_hogar_mujer_no_u6"
    if metric_id == "penalidad_maternidad_trabajo_educ_sup":
        return "penalidad_sup = tasa_trabajo(mujeres con educ. sup. y U6) − tasa_trabajo(mujeres con educ. sup. y sin U6)"
    if metric_id == "penalidad_maternidad_trabajo_no_educ_sup":
        return "penalidad_no_sup = tasa_trabajo(mujeres sin educ. sup. y U6) − tasa_trabajo(mujeres sin educ. sup. y sin U6)"
    if metric_id == "mitigacion_penalidad_trabajo_por_educ":
        return "mitigación = penalidad_maternidad_trabajo_educ_sup − penalidad_maternidad_trabajo_no_educ_sup"
    if metric_id == "penalidad_maternidad_ingreso_educ_sup":
        return "penalidad_sup = ingreso(mujeres con educ. sup. y U6) − ingreso(mujeres con educ. sup. y sin U6)"
    if metric_id == "penalidad_maternidad_ingreso_no_educ_sup":
        return "penalidad_no_sup = ingreso(mujeres sin educ. sup. y U6) − ingreso(mujeres sin educ. sup. y sin U6)"
    if metric_id == "mitigacion_penalidad_ingreso_por_educ":
        return "mitigación = penalidad_maternidad_ingreso_educ_sup − penalidad_maternidad_ingreso_no_educ_sup"
    if metric_id.startswith("share_") and metric_id in JNE_METRIC_IDS:
        return "share = N_mujeres_en_universo_político / N_total_en_universo_político"
    if metric_id == "ratio_autoridades_mujer_hombre":
        return "ratio = N_autoridades_mujeres / N_autoridades_hombres"
    if metric_id == "indice_techo_politico_mujeres":
        return "índice_techo = share_liderazgo_ejecutivo_mujeres / share_autoridades_mujeres"
    if metric_id == "brecha_liderazgo_vs_base_mujeres":
        return "brecha = share_liderazgo_ejecutivo_mujeres − share_autoridades_mujeres"
    if metric_id == "indice_habilitante_liderazgo_femenino_0_100":
        return (
            "z_k = (x_k − min_k) / (max_k − min_k)  [componentes favorables]\n"
            "z_k = 1 − (x_k − min_k) / (max_k − min_k)  [componentes desfavorables]\n"
            "Índice = 100 · promedio(z_k)"
        )
    return "Métrica derivada por agregación, diferencia, razón o normalización según la definición operacional del proyecto."


def infer_metric_variables(metric_id):
    items = []
    if metric_source_key(metric_id) == "ENAHO":
        items.append(("PESO", VARIABLE_DOCS["PESO"]))
        items.append(("SEXO", VARIABLE_DOCS["SEXO"]))
        items.append(("EDAD", VARIABLE_DOCS["EDAD"]))

    if metric_id.startswith("tasa_trabaja_horas_") or metric_id.startswith("horas_prom_") or metric_id.startswith("ratio_trabajo_"):
        items.append(("TRABAJA_HORAS", VARIABLE_DOCS["TRABAJA_HORAS"]))
        items.append(("HORAS_TRABAJADAS_TOTAL", VARIABLE_DOCS["HORAS_TRABAJADAS_TOTAL"]))
    if metric_id.startswith("pobreza_") or metric_id.startswith("ratio_pobreza_") or metric_id.startswith("penalidad_maternidad_pobreza"):
        items.append(("POBRE_BIN", VARIABLE_DOCS["POBRE_BIN"]))
    if "ingreso" in metric_id:
        items.append(("INGRESO_NETO_HOGAR", VARIABLE_DOCS["INGRESO_NETO_HOGAR"]))
    if "educ_sup" in metric_id:
        items.append(("ES_EDUC_SUPERIOR", VARIABLE_DOCS["ES_EDUC_SUPERIOR"]))
    if "educ_univ" in metric_id:
        items.append(("ES_EDUC_UNIVERSITARIA", VARIABLE_DOCS["ES_EDUC_UNIVERSITARIA"]))
    if "u6" in metric_id or "maternidad" in metric_id:
        items.append(("TIENE_NINIO_U6_HOGAR", VARIABLE_DOCS["TIENE_NINIO_U6_HOGAR"]))
    if "ninios_u6" in metric_id or "2mas_u6" in metric_id:
        items.append(("N_NINIOS_U6_HOGAR", VARIABLE_DOCS["N_NINIOS_U6_HOGAR"]))

    if metric_source_key(metric_id) == "JNE":
        items = [
            ("GENERO", VARIABLE_DOCS["GENERO"]),
            ("CARGO", VARIABLE_DOCS["CARGO"]),
            ("AMBITO", VARIABLE_DOCS["AMBITO"]),
            ("UBIGEO", VARIABLE_DOCS["UBIGEO"]),
            ("ANIOELECCION", VARIABLE_DOCS["ANIOELECCION"]),
            ("ORGANIZACIONPOLITICA", VARIABLE_DOCS["ORGANIZACIONPOLITICA"]),
        ]

    if metric_source_key(metric_id) == "COMBINED":
        items = [
            ("Componentes ENAHO", "Educación, trabajo, ingreso, pobreza y cuidados procesados con pesos muestrales."),
            ("Componentes JNE", "Representación y liderazgo femenino agregados por departamento y año electoral."),
            ("Normalización min-max", "Escalamiento de cada componente a [0, 1] antes de promediar."),
        ]

    # preservar orden y evitar repetidos
    seen = set()
    out = []
    for k, v in items:
        if k not in seen:
...
def infer_metric_official_note(metric_id):
    if metric_source_key(metric_id) == "ENAHO":
        if metric_id.startswith("pobreza_") or "ratio_pobreza" in metric_id or "penalidad_maternidad_pobreza" in metric_id:
            return (
                "La condición de pobreza proviene del marco oficial del INEI. En el dashboard se usa la variable ya codificada en tu base analítica, "
                "sin reestimar la línea de pobreza dentro de la app."
            )
        return (
            "La métrica se deriva directamente de microdatos o variables derivadas de la ENAHO, empleando ponderación por PESO y los filtros poblacionales del proyecto."
        )
    if metric_source_key(metric_id) == "JNE":
        return (
            "La métrica se deriva de conteos y proporciones sobre registros de autoridades electas. "
            "La regla exacta de agrupación por cargo/ámbito corresponde al preprocesamiento de tu proyecto."
        )
    return (
        "La métrica es una integración analítica propia del proyecto; no existe como indicador oficial prepublicado, pero es reproducible a partir de fuentes oficiales."
    )


def build_metric_methodology_card(metric_id):
    src = SOURCE_DOCS[metric_source_key(metric_id)]
    sem = metric_semantics(metric_id)
    description = infer_metric_description(metric_id)
    formula = infer_metric_formula(metric_id)
    variables_md = _md_bullets(infer_metric_variables(metric_id))
    note = infer_metric_official_note(metric_id)
...
### Fuente oficial base
- **Fuente:** {src['name']}
- **Institución:** {src['institution']}
- **Descripción oficial resumida:** {src['description']}
- **Cobertura usada en el dashboard:** {src['coverage']}
- **Nota metodológica:** {src['note']}
- **Derivación de esta métrica:** {note}

### Interpretación
- **Lectura sustantiva:** {sem['reading']}
- **Punto de referencia:** {sem['range_hint']}
"""
    ...
    return dbc.Card(
        dbc.CardBody(
            [
                html.Div(
                    [
                        html.Div("Ficha técnica de la métrica", style={"fontWeight": "800", "fontSize": "1.05rem", "color": TEXT}),
                        html.Div(metric_label(metric_id), style={"color": PURPLE, "fontWeight": "700", "marginTop": "2px"}),
                    ],
                    style={"marginBottom": "10px"},
                ),
                dcc.Markdown(md, style={"color": TEXT, "fontSize": "0.95rem"}),
            ]
        ),
        style=card_style(),
    )
            ...
            [
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                html.Div("Bienvenida", style={"fontWeight": "800", "fontSize": "1.1rem", "color": TEXT}),
                                html.Div(
                                    "Este dashboard integra información socioeconómica, educativa, de cuidados y representación política para explorar cómo se articulan las brechas de género en el territorio.",
                        ...
                        dbc.Col(
                            [
                                html.Div("¿Por qué se hace?", style={"fontWeight": "800", "fontSize": "1.1rem", "color": TEXT}),
                                html.Div(
                                    "Porque la desigualdad de género no aparece en una sola variable: se expresa en trabajo, educación, pobreza, cuidados y acceso a espacios de poder. El objetivo es mostrar esas dimensiones de forma integrada y comparable.",
                                    style={"color": MUTED, "marginTop": "6px"},
                                ),
                            ],
                            md=4,
                        ...
                        dbc.Col(
                            [
                                html.Div("Fuentes oficiales", style={"fontWeight": "800", "fontSize": "1.1rem", "color": TEXT}),
                                html.Div(
                                    "ENAHO - INEI para el componente socioeconómico y de cuidados; JNE para el componente de autoridades electas y liderazgo político.",
                                    style={"color": MUTED, "marginTop": "6px"},
                                ),
                            ],
                            md=3,
            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        html.Div(id="metric-methodology-box", style={"marginBottom": "12px"}),
...
@app.callback(
    ...
    Output("metric-methodology-box", "children"),
    ...
    try:
        ...
        if dep_df.empty or nat_df.empty:
            ...
            return (
                ...
                dbc.Card(dbc.CardBody("Sin ficha técnica disponible."), style=card_style()),
        ...
        if value_col not in dep_year.columns:
            ...
            return (
                context_note,
                metric_note,
                build_metric_methodology_card(metric_id),
        ...
        metric_methodology = build_metric_methodology_card(metric_id)
        ...
        return (
            ...
            dbc.Card(dbc.CardBody("Error en ficha técnica de la métrica."), style=card_style()),
